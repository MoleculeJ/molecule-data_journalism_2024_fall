...
First: an aside on geographic data. There are many formats for geographic data, but data type you'll see the most is called the shapefile. It comes from a company named ERRI, which created the most widely used GIS software in the world. For years, they were the only game in town, really, and the shapefile became ubiquitous, especially so in government and utilities.
...
**Answer** The code imported dataframe with columns containing data on the precinct's features like ID, name, congress, legislative, council, school, and polling' ID, and columns relating to the precinct's geography like 'shape length', 'shape area', and 'shape'.  
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** Most of the columns like 'ID', 'congress', 'legis', etc. contain numeric data, and column 'name' has string data, while column 'shape' has geometry data.  
...
**Answer** We got the map of Prince George's County divided by shape representing precincts' territories. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** Nothing appears. I assume that it just means 'tigris' has been installed. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We got a new dataframe that contains data of every county in the U.S. and their properties like states and counties FIPS codes, GEOID, geometry data, etc. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We got a new dataframe that contains only rows of counties in Maryland.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** We got the map of Maryland divided by shape representing counties. Nonetheless, it seems like the map leaves out the separation between land and the bay area. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** It looks like we got the Prince George's County map divided by precinct boundaries on top of another Prince George's map as background, but there are some areas on the bottom left of the top layer that do not fully overlap with the background layer.  
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We got the new dataframe showing votes turnout data in Prince George's County categorized by precincts. Each row represents 1 precinct with data on the total of votes at the poll, early votes, absentee, provisional votes, and all the eligible voters.    
...
**Task** Run the following code. Describe the output in the space below.
**Answer** We got the same precinct dataframe plus voter turnout data. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We got the Prince George's Map consisting of each precinct area that is filled with color representing the number of eligible voters in each precinct. We also see the precincts that have no population with zero (white color) eligible voters.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We got a new dataframe containing data of each county's voters categorized by party affiliation.  
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We got a new dataframe with the population data of each county in Maryland.  
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We got the same population dataframe with one additional column of geometry data. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** The code displays dataframe showing voters from each Maryland County. Values in the County column spell out only the name of the county. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We got the new version of the population dataframe. The 'NAME' column has been changed to 'County' with the values that spell out only the county name in the form of titilecase with no white space behind it. The 'estimate' column has been changed to 'population'. Lastly, this new version of the population dataframe contains only columns that we intend to use which are 'County', 'population', and 'geometry'.    
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We got the joint version of population and voters by county dataframe, but values in Baltimore and St. Mary's row are missing. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** The values in the 'County' column of the population dataframe have been changed from St. Mary's to Saint Mary's, and from Baltimore to Baltimore County.   
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** Now, we can join population and voter by county dataframe without missing values. We create a new column that contains values of the number of voters per 10,000 population for each county which are used to arrange the order of the whole dataframe from highest to lowest.    
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We got the Maryland map divided by counties' areas that are filled with color representing the number of voters per 10,000 population.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We got the same map with different color scales from the blue shade to the scale consisting of black, dark blue, purple, magenta, orange, and pale yellow.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** Counties with the most number of voters per 10,000 population like Worcester, Queen Anne's, and Talbot are located on the eastern shore, and the County with the lowest number of voters per 10,000 population like Somerset is also located on the eastern shore area as well.   
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We got the same dataframe plus a new column containing values of the number of unaffiliated voters per 10,000 population. 
...
**Answer** Counties with the most number of unaffiliated voters per 10,000 population like Frederick, Howard, and Anne Arundel are located in the center of the state.
