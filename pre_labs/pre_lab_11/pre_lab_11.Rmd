...
library(tidyverse)
library(janitor)
#You should bring 'janitor' along every time you scrape things because you will want the column name to be changed.
```
...
**Answer** The columns have long headers with lots of white space. It may be not convenient to use them. 
...
**Answer** I think each number represents the order of column headers counting from left to right.   
...
results <- results[[1]] |>
  ...
  #you can put the ordinal number of column instead of old name 
  slice(-25) |>
  #this means get rid of 25th row / - means get rid of / if you type slice(25) it will give you only row 25th.
  mutate(harris = as.numeric(gsub(",","", harris))) |>
  #this means change the datatype to numeric  and get rid of comma then replace it by nothing.
...
**Answer** There are 55 <li> tags in this HTML source. 

```{r}
cmte_results |> html_elements('li')
#element = grab only first list 
#elements = grab every list
```
...
**Answer** The code shows text containing inside each <li> tag, so now we can see which <li> tags are the names of committees. 
...
**Answer** We got the dataframe with 2 columns. First column named "name" contains the text name of each <li> tag and second column named "url" contains url of each <li> tags' site.  

```{r}
committees <- cmte_results |> html_elements("li a")
#grab 2 elements which are <li> tags and <a> tags. (we want <a> tags because url of each commitee's site is in this element) 

committees_with_urls <- tibble(
  #make this dataframe
  name = committees %>% html_text(trim = TRUE),
  #Name the first column header as "name" and then trim all the white space of the values (the values in this column is gonna be the text of each committee's name)
  url = committees %>% html_attr("href")
  #Name the second column header as "url" and grab the value from "href". "href" is attribution in <a> tag containing url.
)
```
...
# We want only the rows of committee so we have to filter out rows that begin with a backslash character which are the rows that is not committee. 
```
