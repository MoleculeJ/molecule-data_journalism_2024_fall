...
```{r echo=FALSE}
census_api_key("950c4cbc9a08989e759e33f664d6d76b87394200", install=TRUE)
```
...
Is the difference between Calvert and Montgomery significant? **Answer** Not that much because their median household incomes have margins of error that overlap, so it's hard to conclude which one is richer.  

Is the difference between Howard and everyone else significant? **Answer** Yes, mainly because its median household income has a margin of error that doesn't overlap with other counties' margin of error, so we're likely to be able to conclude that it's the richest county in Maryland.  
...
**Answer**
- No county has a decreased median income.  

- I am not sure how to answer the second part of the question with the result from your previous code, so I did a little more research to find a way to calculate. I am not sure if this is correct but I found that we can calculate the lower and upper bound of percentage change to see whether the percentage change falls in this range or not. If so, that means the change is not outside the MOE of both years. 

From the result, my answer is no change from any county is outside the margin of error for both years.
...
md10 |>
  ...
  arrange(change)
#I am not sure how to answer the second part of the question with the result from your previous code, so I did little more research to find the way to calculate, and write a few more code to find the answer. 
md10 |>
  inner_join(md22, join_by(GEOID, NAME)) |>
  rename(estimate2010=estimate.x, estimate2022=estimate.y) |>
  mutate(min2010 = estimate2010-moe.x, max2010 = estimate2010+moe.x, min2022 = estimate2022-moe.y, max2022 = estimate2022+moe.y) |>
  mutate(change = ((estimate2022-estimate2010)/estimate2010)*100) |>
  mutate(max_possible_increase = ((max2022-min2010)/min2010)*100, min_possible_increase = ((min2022-max2010)/max2010)*100)|>
  select(NAME, change, max_possible_increase, min_possible_increase)
```
...
**Answer** 21202 had the 19th-largest amount, with $48,784 as the estimated median household income. 
...
**Answer**
- Why do areas in the zip codes like 21723 or 21071, which were among the top ten areas with the highest median household income, have so little amount of contributions like $1,500 (in 21723) or $50 (in 21071)? Who were these rich people that didn't want to spend money for political reasons and why? 
- On the contrary, who were the people in areas like 20854 or 20817 that were rich people that were ready to spend large amounts of money for political reasons and why?  
