...
**Answer** Yes, all of them have the same number of columns written in the exact same column headers.  
...
#   filter(!COUNTY == "TOTAL")
# to get values that are NA in the columns
#   filter(is.na(COUNTY))
# to get every value that are not NA in the columns
#   filter(!is.na(COUNTY))
```
...
maryland_population |> 
  ...
  nrow()
#full_join (outer join) could be used when we want to recheck if data is bad like there are rows that shouldn't be in there, or there are rows that are missing, or you do initial join and receive fewer rows that you expect to get. 
```
...
**Answer** The message says that the code joins 2 spreadsheets by the column name 'COUNTY'. (Joining with `by = join_by(COUNTY)`)
...
**Answer** The answer is Prince George's County.
...
**Answer** The highest percentage of the Asian population over 18 is in Howard County, and all the percentages from each County are more clustered spanning from about 0.3% to 19.36% which is different from the percentage of the Black population over 18 from each County which is less clustered spanning from 0.78% to 60.90%. 
...
**Answer** Its data type is number. 
...
**Answer** Its data type is character. 
