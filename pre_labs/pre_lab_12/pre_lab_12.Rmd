...
**Answer** We got the dataframe that looks familiar to the one from lab 11 with one new column, 'tex', that contains the whole text of each press release. 
...
**Answer** It counts the same words that are written with different cases as different words and also counts commas as a word.  
...
**Answer** We got a dataframe with one column, 'word', that contains every word that is not punctuation in lowercase from column 'text' in the previous dataframe. 
...
**Answer** It shows that the most unique word found in all press releases is "the", and no it's not that interesting to me haha. 
...
**Answer** It shows that the most unique word found in all press releases is "health". Yes, it's a little bit more interesting than the previous result.
...
**Answer** In 2022, the the most unique word found is 'health', and in 2024, the most unique word found is 'baltimore'.
...
**Answer** It shows that the most two-word phrase found in the 2022 press releases is 'amount included'. 
...
**Answer** It shows that the most two-word phrase found in the 2024 press release is 'health care'.
...
library(devtools)
install_github("trinker/textstem")
...
**Answer** Stemming detects 'drive', 'drives', and 'driving' as the same word, but distinguishes 'drive' in other tense forms ('drive', 'driven') as a different word, while lemmatization sees every word except 'driver' as the same word. I think if the specificity in terms of tense is not relevant, lemmatization might be more useful in this analysis.    
...
**Answer** I think the first one is more useful because the second result shows the count of some words that aren't significantly different in meaning and are supposed to be detected as the same thing like 'fund' and 'finding'.   
...
**Answer** We got the dataframe with 'word', 'sentiment', and 'n' columns. I think some words should be described as neutral because without context it might seem hard to tell if they are used in positive or negative sentiment such as important, top, consistent, competitive, rapid, etc.
