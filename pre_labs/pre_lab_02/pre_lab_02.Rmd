...
### Task 1: Load libraries
 to load the tidyverse library. To run the code, click the little green play button (left facing arrow) at the top right of the codeblock. In Rmarkdown data notebooks, we write code inside of codeblocks, and explanatory text in the white area outside of it.
...
**Answer** The code brings only 7 columns, office_name, office_district, candidate_name, party, county_name, election_day, and votes, from the original dataset to display, and creates a new column named 'percent_election_day' by dividing values in 'election_day" by values in 'votes' and putting the results in 'percent_election_day'.  
...
**Answer** Every row is rearranged by the values in the 'percent_elcetion_day' column in descending order which is different from rows in a spreadsheet in the previous task that is arranged randomly. This is because the 'arrange(desc())' function rearranges the order of rows shown in this dataset. 
...
**Answer** I think so because almost all of the top 100 candidates who have least percentage of votes on election day are democratic candidates, so it could be used to write stories about why democratic supporters tend to cast their vote via other means than at the box on election day.    
...
**Answer**
- str_to_lower() = converts all the letters in the word to lowercase.
- str_to_title() = converts the word to title case, where only the first letter of each word is capitalized.
...
**Answer** The mutate statement picks up the values in column 'amount' and categorizes them based on their calculable values into 5 groups of conditions which are labeled as "Less than $100" for values below 100, "Between $100 and $499" for values that equal to or more than 100 but less than 500, "Between $500 and $1499" for values that equal to or more than 500 but less than 1500, "Between $1500 and $2999" for values that equal to or more than 1500 but less than 3000, and "$3,000 or more" for values that equal to or more than 3000. After that, the code will display these labels categorizing the values in 'amount' of each row in the new column called 'amount_category'.    
...
**Answer** It is 'Less than $100' category. 
...
**Answer** If we do not assign the result to a new variable, the new spreadsheet showing only columns that we want to work on will not be permanent, and we will not be able to export it for further usage. 
...
**Answer:** The data type below the column name of 'expenditure_date' has changed from <chr> to <date> signifying that the data type of values in this column has changed from character to date. 
...
**Answer** I think the values in the 'month' column are the grouping of rows that share the same month within the same year together, and differentiate the rows that share the same month but have different years from each other. 