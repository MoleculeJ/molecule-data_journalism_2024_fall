...
**Answer** It says "Warning: One or more parsing issues, call `problems()` on your data frame for details, e.g.:".
...
**Answer**
- Based on the number of rows, indicates that there are 1,640 rows that could be problematic. 
- The problem, as shown in these 1,640 rows, is that columns 12th and 13th ('provisional', 'limited') have numeric values in some rows while all of them are supposed to be NA (lgl data type), as 'guess_max' function took a guess from estimating first 10 rows of the columns. When the problems() function detects values that are not in line with what 'guess_max' assumes, it demonstrates those values as a problem.
...
**Answer** There are no errors appear. 
...
**Answer**
- The data type of "district" is number. 
- The data type of "precinct" is character. 
- The data type of "mail" is number. 
...
**Answer**: The result means that there are 402,345 precincts that have no data on how many people vote via mail. 
...
**Answer**
- There are 12 registrations for January 1900. 
- The first 20 rows could indicate that the oldest age range of people who register for the vote in Yadkin County is approximately 75 - 142 years old which seems impossible, so this could mean some people might make typos or some errors might occur during the registration.   
...
**Answer** Yes, I think these data are worthy of exploring more because they could lead to some interesting questions or even could be developed to become stories, for instance;
- Some statuses are not self-explanatory like LEGACY DATA, so what does it mean? 
- How come people have DECEASED status, and are their votes still counted after they die? 
- As I don't fully understand the meaning of all status, which status means you can still vote and which means you cannot? 
...
**Answer**
- Although they look like numbers, the values' data type in the 'amount' column are characters, so we will not be able to calculate them. 
- Although they appear as the same city, the ways values in the 'city' column are written are different in terms of format and spelling. This inconsistency could be a problem if we try to 'group_by' and count them.
- The format of values in the 'zip' column is not consistent. Some are 5 digits and some are 9 digits. Hence, this could be a problem if we try to 'group_by' and count them.
...
**Answer** The code put x in front of the old column name which starts with 1 because the column name should not start with a numeric character. 
...
**Answer** The error said 
"in `summarise()`:
ℹ In argument: `total_amount = sum(amount)`.
Caused by error in `sum()`:
! invalid 'type' (character) of argument"

This error means that the code cannot calculate the sum for values in the 'amount' column because they are characters, not calculable numbers. 
...
**Answer** There two rows that look exactly the same, and the donor name of these rows is Derrick Hamilton.
...
**Answer** The two rows that previously had 9-digit values in the 'zip' are cut down to have only the first 5 digits left making all the values in this column the same. 
...
**Answer** There are 2 misspellings of Conowingo left which are 'Conowing' and 'Conowingoo'.
...
**Answer** There are 1 misspelling of Conowingo left which is 'Conowingoo'.
...
**Answer** There is no misspellings left. 
