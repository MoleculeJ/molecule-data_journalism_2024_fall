...
**Answer**
- Data in the first row are supposed to be values, not the headers. 
- Many rows are blank / have no data (NA).
- Some information is supposed to be on different columns such as the party behind the candidates' names. 
...
**Answer** 
- The party abbreviations are still mixed up with the candidates' names. 
- Other columns beside the 'candidate' still don't have the appropriate name. 
...
benzie_2024 <- benzie_2024 |>
  filter(!is.na(X2)) |> 
# I want to keep every thing that is not NA / I want to get rid of every NA
  rename(candidate = X1)
# New name comes first 
benzie_2024
```
...
**Answer** 
- There are 2 new rows which are 'office' and 'party', and party information isn't mixed up with candidates' names anymore.
- Many columns still don't have the appropriate names and as we have not assigned them yet, so many rows still don't have data on the 'office' column.
- There is a row that doesn't have values on 'office, 'candidate' and 'party' with 0 values on every other column. I think we could delete this column. 

```{r}
benzie_2024 <- benzie_2024 |>
  separate(candidate, c("candidate", "party"), sep=' - ') |> 
# Separate 'candidate' into 'candidate' and 'party', Separate when you found ' - ' 
