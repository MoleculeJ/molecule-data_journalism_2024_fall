...
**Answer**
- Firstly, the code made a change to the datafreame 'md_voters' by creating new a column named 'UNAF_DIFF' which stored values that were the outcomes of subtracting values in UNAF_2024 by values in UNAF_2022.
- Secondly, the code made changes to the 'md_voters' and saved it in a new dataframe called 'maryland_top_counties'. These changes included arranging the dataframe based on values in UNAF_DIFF from max to min, selecting to display only 'County' and 'UNAF_DIFF' columns, and filtering to show only the first 10 rows. 
- Thirdly, the code illustrated the 'maryland_top_counties'.
...
**Answer** The empty grey area. 

```{r}
maryland_top_counties |>
  ggplot() #Create blank space that I can draw something on
```
...
**Answer** I see the bar chart. Each bar represents the counties and their heights represent the values in the 'UNAF_DIFF' column. All the bars are arranged alphabetically from left to right by the counties' names.  
...
**Answer** All the bars are rearranged by their height from the shortest one on the left to the tallest one on the right.  
...
**Answer** All the bars are flipped from vertical form to horizontal form, and the labels which are the names of counties each bar represents are much easier to read now.  
...
**Answer** The grey background are gone. 
...
**Answer** The code adds a turquoise background to the chart and changes the font into the Economist font style.
...
**Answer** The code adds up the title of the chart, changes the x- and y-axis names, and adds the caption on the bottom left of the chart. 
...
maryland_top_counties |>
  ...
  theme_economist() +
  labs(
    title="Change in Unaffiliated Voters, 2022-2024",
    y = "net change",
    x = "county",
...
**Answer** I see the dataframe showing two columns of 'date' and 'total_contributions'. The rows in dataframe are arranged based on values in the 'date' column chronologically from 2022-04-01 to 2022-06-30.
...
**Answer** I see the line chart showing a line that fluctuates from left to right approximately between 400 to 800 until it peaks suddenly at the end reaching about 1100 at the end of June. The x-axis segmented by tick marks of months represents the date of each point on the line and the y-axis represents the contributions of each point on the line.   
...
**Answer** The code changes the x-axis that used to be segmented by month to be divided by tick marks that have month and date labels instead. 
...
**Answer** The code tilts the labels of the x-axis 45 degrees making them easier to read. 
